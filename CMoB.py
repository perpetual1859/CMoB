...
from datasets.dataloader_AVE import AV_KS_Dataset, AV_KS_Dataset_sample_level
...
def get_arguments():
    ...
    parser.add_argument("--n_classes", default=28, type=int)
    ...
    with torch.no_grad():
        ...
        for step, (image, spec, label, index) in tqdm(enumerate(dataloader)):
            ...
            for i, item in enumerate(label):
                ...
                #add
                if index_all == label[i] and ALL_value > V_value and ALL_value > A_value:
                # if index_all == label[i]:
                    value_all = 2.0
                if index_v == label[i]:
                    value_v = 1.0
                ...
                # 修改
                # contrib_a =  value_all - value_v
                # contrib_v =  value_all - value_a
                cona += contrib_a
                conv += contrib_v
...
def train_epoch(args, epoch, model, device, dataloader, optimizer,cona,conv):
    criterion = nn.CrossEntropyLoss()

    def minimize_difference_loss(cona_min, conv_min):
        return torch.abs(cona_min - conv_min)
    ...
    for step, (image, spec, label, index, drop) in tqdm(enumerate(dataloader)):
        ...
        #add abs|cona-conv|
        # cona_min = torch.tensor(cona, requires_grad=True)
        # conv_min = torch.tensor(conv, requires_grad=True)

        # diff_loss = minimize_difference_loss(a.mean(), v.mean())
        # diff_loss = torch.nn.L1Loss(cona_min, conv_min)
        ...
        for epoch in range(args.epochs):
            ...
            if epoch < args.warmup:
                ...
                batch_loss = train_epoch(
                    args, epoch, model, device, train_dataloader, optimizer,cona,conv
                )
            ...
            if acc > best_acc:
                ...
                model_name = "{}_best_model_of_{}_{}_{}_{}_epochs{}_batch{}_lr{}_en{}_warmup{}_bottom{}.pth".format(
                    args.compare,
                    args.optimizer,
                    args.dataset,
                    args.modulation,
