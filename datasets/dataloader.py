...
class AV_KS_Dataset_sample_level(Dataset):
    def __init__(self, mode, contribution, transforms=None):
        ...
        for i in range(length):
            ...
            # if 0.4 < contrib_v <= 1:  # 0.5
                for tt in range(1):
                    # self.data.append(self.data[i]) 改
                    # mask_rate = 0.3
                    # masked_image = self.apply_mask(self.data[i])
                    # self.data.append(masked_image)
                    self.data.append(self.data[i])
                    self.label.append(self.label[i])
                    self.mask.append(1)
                    self.drop.append(1)
    ...
    #掩码
    def apply_mask(self,image, mask_rate):

        mask = Image.new('L', image.size, 0)  # 初始化掩码为黑色（全零）

        # 计算掩码区域的大小
        height, width = image.size
        mask_height = int(height * mask_rate)
        mask_width = int(width * mask_rate)

        # 随机选择掩码的起始位置
        x = random.randint(0, width - mask_width)
        y = random.randint(0, height - mask_height)

        # 使用Pillow的ImageDraw在掩码上绘制白色矩形
        draw = ImageDraw.Draw(mask)
        draw.rectangle((x, y, x + mask_width, y + mask_height), fill=255)

        # 应用掩码
        masked_image = Image.composite(image.convert('RGB'), mask.convert('L').convert('RGB'), mask=mask)

        return masked_image

    def apply_mask_pytorch(self,image, mask_rate):
        ...
        random_mask = np.random.rand(height, width) < mask_rate
    ...
    def __getitem__(self, idx):
        ...
        #     image = Image.open(img_path).convert('RGB')
            # if self.mask[idx]==1:
            #     mask_rate =0.1
            #     image=transf(image).unsqueeze(0).float()
            #     # image_arr.append(transf(image).unsqueeze(0).float())
            #     image = self.apply_mask_pytorch(image, mask_rate)
            #     image_arr.append(image)
            # elif self.mask[idx] == 2:
            #     mask_rate = 0.4
            #     image = transf(image).unsqueeze(0).float()
            #     image = self.apply_mask_pytorch(image, mask_rate)
            #     image_arr.append(image)
            # elif self.mask[idx] == 3:
            #     mask_rate = 0.7
            #     image = transf(image).unsqueeze(0).float()
            #     image = self.apply_mask_pytorch(image, mask_rate)
            #     image_arr.append(image)
